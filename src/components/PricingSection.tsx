...
const PricingSection = ({}: PricingSectionProps) => {
  ...
  const packages = [{
    ...
    features: ["Kundenanalysen", "Marktauswertungen", "Erfolgstracking", "Monatlicher Gewinntreiberreport", "Abweichungsanalyse ggü. Plan", "Finanzmodelle", "Szenariomodellierungen", "Mittel- und Langfristplanung"]
  }, {
    ...
    icon: Wrench,
    color: "from-green-600 to-green-700",
    features: ["Kundenanalysen", "Marktauswertungen", "Erfolgstracking", "Monatlicher Gewinntreiberreport", "Abweichungsanalyse ggü. Vorjahr"]
  }, {
  ...
  return <section className="py-20 bg-background">
      <div className="container mx-auto px-6">
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-muted-foreground mx-auto mb-8">Kontinuierliche Fachexpertise ohne teure Neueinstellungen</p>
        </div>

        <div className="grid lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
          {packages.map((pkg, index) => {
          const IconComponent = pkg.icon;
          return <div key={index} className={`relative bg-card rounded-2xl p-8 card-shadow hover:elegant-shadow smooth-transition hover:-translate-y-1 flex flex-col ${pkg.name === "Angebotsoptimierung" ? "ring-2 ring-green-600" : ""}`}>
                {pkg.name === "Angebotsoptimierung" && <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                ...
                const shouldNotBeBold = isAngebotsoptimierung && ["Kundenanalysen", "Marktauswertungen", "Erfolgstracking"].includes(feature) || isAngebotsstrategie && ["Kundenanalysen", "Marktauswertungen", "Erfolgstracking", "Monatlicher Gewinntreiberreport"].includes(feature);
                return <div key={featureIndex} className="flex items-center space-x-3">
        ...
        <div className="max-w-7xl mx-auto mt-16">
          <Collapsible open={isOpen} onOpenChange={setIsOpen}>
            ...
            <CollapsibleContent className="overflow-hidden data-[state=open]:animate-accordion-down data-[state=closed]:animate-accordion-up">
              <p className="text-lg text-muted-foreground mb-4">Höhere Umsätze bedeuten grösseres Gewinnpotential</p>
